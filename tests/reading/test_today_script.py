...
def _run_today(tmp_path, top_n=20, extra_args=None):
    ...
    cmd = [sys.executable, "-m", "auto.reading.cli.today",
           "--config", str(REPO_ROOT / "modules" / "reading" / "config" / "research_interests.yaml"),
           "--output", str(output),
           "--top-n", str(top_n)]
    if extra_args:
...
def test_envelope_module_field_is_reading(tmp_path):
    rc, data = _run_today(tmp_path, top_n=5)
...
def test_today_emits_log_event(tmp_path, monkeypatch):
    """today.py must write at least one log_event to ~/.local/share/auto/logs/<date>.jsonl"""
    ...
    reading_events = [e for e in events if e.get("module") == "reading"]
    assert len(reading_events) >= 1, f"no reading events in {events}"
    event_names = {e.get("event") for e in reading_events}

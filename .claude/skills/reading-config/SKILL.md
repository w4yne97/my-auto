...
通过对话引导用户完成 vault 初始化（首次使用）或查看/修改现有配置。所有配置存储在 `$VAULT_PATH/00_Config/research_interests.yaml`。
...
根据收集到的信息生成 `$VAULT_PATH/00_Config/research_interests.yaml`：
...
读取 `$VAULT_PATH/00_Config/research_interests.yaml` 并格式化展示：
...
3. 写回 `$VAULT_PATH/00_Config/research_interests.yaml`
4. 展示修改后的配置确认

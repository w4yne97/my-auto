...
1. **定位概念**：在 `~/.local/share/start-my-day/auto-learning/knowledge-map.yaml` 中查找匹配的概念（支持模糊匹配中文名或路径）
2. **读取学习路线**：检查 `~/.local/share/start-my-day/auto-learning/learning-route.yaml` 中该概念的位置
3. **构建溯源链**：
   - 从 `modules/auto-learning/config/domain-tree.yaml` 读取该概念的 `prerequisites`
...
使用 `modules/auto-learning/lib/templates/study-session.html` 模板，填充以下内容：
...
4. 给出建议的 depth 评估
5. 提示下一步：根据 learning-route.yaml 展示下一个应学概念

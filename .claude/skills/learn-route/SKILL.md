...
`learning-route.yaml` 是缓存路线和展示层，不是学习事实源。事实源是：

- `modules/learning/config/domain-tree.yaml`：静态概念图谱、prerequisites、默认 priority/target。
- `~/.local/share/auto/learning/knowledge-map.yaml`：当前 depth、confidence、status、学习证据状态。

生成或展示路线前，应使用 `auto.learning.validation` 的语义检查 domain-tree 和 route/knowledge-map 漂移。每日推荐优先使用 `auto.learning.planner.plan_next_concepts()` 动态计算，route 只提供连续性信号。
...
1. **读取数据源**：
   - `modules/learning/config/domain-tree.yaml` — 获取 prerequisites 依赖关系
   - `~/.local/share/auto/learning/knowledge-map.yaml` — 获取当前 depth、target_depth、priority、confidence
...
6. **写入 `~/.local/share/auto/learning/learning-route.yaml`**：
...
1. 从 `auto.learning.planner.plan_next_concepts()` 取最高分候选
2. 同时展示它在 cached route 中的位置（如果存在）
3. 展示该概念的完整溯源链：`root → ... → current → [NEXT]`
4. 检查溯源链中所有 prerequisites 的完成状态
5. 如果 route 第一 pending 项不是 planner 最高分候选，解释差异：gap/priority、前置完成度、route 漂移或上下文连续性
6. 输出建议：运行 `/learn-study <concept>` 开始学习
...
- **`/learn-plan`**：每日计划基于 planner 动态生成，route 只作为连续性信号
- **`/learn-progress`**：更新进度后重新校验 route 是否漂移；可以更新 route cache，但 knowledge-map 是事实源
